...
class AtaqueCoberturaDependencia {
  constructor({ tamanhoBloco = 32, perturbacoesPorPar = 5 } = {}) {
  ...
  executar(alvo) {
    ...
    const ksBase = alvo.gerarKeystreamBruto(chaveBase, iv, 'enc', tamanho);
    if (ksBase == null) {
    ...
    for (let posEntrada = 0; posEntrada < tamanho; posEntrada++) {
      const afetouAlgumaSaida = new Array(tamanho).fill(false);
...
module.exports = AtaqueCoberturaDependencia;
