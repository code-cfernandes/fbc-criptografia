...
const { randomInt } = require('../Util.js');
...
class AtaqueCoberturaDependenciaIV {
  constructor({ tamanhoBloco = 32, perturbacoesPorPosicao = 5 } = {}) {
  ...
  executar(alvo) {
    ...
    const ksBase = alvo.gerarKeystreamBruto(chave, ivBase, 'enc', this.tamanhoBloco);
    if (ksBase == null) {
      throw new SkipAtaqueException('Alvo não expõe gerarKeystreamBruto.');
    }

    const paresIndependentes = [];

    for (let posIv = 0; posIv < tamanhoIv; posIv++) {
      const afetou = new Array(this.tamanhoBloco).fill(false);
...
module.exports = AtaqueCoberturaDependenciaIV;
