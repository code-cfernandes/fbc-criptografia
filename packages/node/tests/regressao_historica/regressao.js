...
const Criptografia = require('../../src/Core/Criptografia.js');
...
function passoComBug(bug, a, b, keyBuf, propositoBuf, posFib, roundIdx) {
...
function gerarKeystreamComBug(bug, keyBuf, ivBuf, proposito, tamanho) {
  if (bug === 1) {
    // bug 1: keystream ignora o IV (vaza o mesmo fluxo para o mesmo plaintext).
    return Criptografia._internal.gerarKeystream(keyBuf, Buffer.alloc(ivBuf.length), proposito, tamanho);
  }
...
function checksumComBug(bug, dadosBuf, keyBuf) {
  ...
  for (let rodada = 0; rodada < 8; rodada++) {
    ...
    for (let i = 0; i < dadosBuf.length; i++) {
      ...
      acumulador = Criptografia._internal.rotEsquerda32(acumulador, (i % 13) + 1);
    }
    if (bug !== 3) {
      for (let k = 0; k < 3; k++) {
        acumulador = (acumulador ^ (acumulador >>> 16)) >>> 0;
        acumulador = Number((BigInt(acumulador) * 16777619n) & 0xffffffffn);
        acumulador = Criptografia._internal.rotEsquerda32(acumulador, 13);
      }
    }
...
function base64urlDecodeComBug(bug, str) {
  if (bug !== 6) {
...
class SnapshotAlvo extends CriptografiaAlvo {
  ...
  encrypt(texto) {
    ...
    const textBuf = Buffer.from(texto, 'utf8');
    ...
    return 'FBC' + Criptografia.base64urlEncode(Buffer.concat([integridade, ct, iv]));
  }
...
for (const caso of CASOS) {
  ...
  try {
    ...
    console.log(`  ❌ bug ${caso.bug} (${caso.descricao}): erro — ${e.message}`);
    falhas++;
    continue;
  }
