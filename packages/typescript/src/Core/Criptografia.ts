...
function rotEsquerda32(val: number, n: number): number {
  n &= 31;
  val = val >>> 0;
  if (n === 0) return val >>> 0;
...
function gerarKeystream(keyBuf: Buffer, ivBuf: Buffer, proposito: string, tamanho: number): Buffer {
...
function xorBytes(dadosBuf: Buffer, keystreamBuf: Buffer): Buffer {
...
function checksum(dadosBuf: Buffer, keyBuf: Buffer): Buffer {
...
export function encrypt(text: string | Buffer): string {
  ...
  const textBuf = typeof text === 'string' ? Buffer.from(text, 'utf8') : Buffer.from(text);
...
export const _internal = { gerarKeystream, checksum, rotEsquerda8, rotEsquerda32 };
