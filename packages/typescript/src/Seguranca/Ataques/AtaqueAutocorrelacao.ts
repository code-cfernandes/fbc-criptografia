...
import { randomBytes, contarBitsBuffer } from '../Util.ts';
 ...
 */
export class AtaqueAutocorrelacao implements AtaqueInterface {
  private readonly tamanho: number;
  private readonly tamanhoBloco: number;

  constructor({ tamanho = 8192, tamanhoBloco = 32 }: { tamanho?: number; tamanhoBloco?: number } = {}) {
  ...
  executar(alvo: AlvoCriptografico): ResultadoAtaque {
    ...
    const suspeitos: Record<number, number> = {};
  ...
  private correlacao(s: Buffer, lag: number): number {
    const n = s.length;
    if (n <= lag) {
      return 0.0;
    }
