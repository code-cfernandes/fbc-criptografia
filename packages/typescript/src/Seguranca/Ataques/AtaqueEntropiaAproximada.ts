...
interface OpcoesEntropiaAproximada {
  tamanho?: number;
  m?: number;
  controles?: number;
  amostrasKeystream?: number;
  limiteZ?: number;
}
 ...
 */
export class AtaqueEntropiaAproximada implements AtaqueInterface {
  private readonly tamanho: number;
  private readonly m: number;
  private readonly controles: number;
  private readonly amostrasKeystream: number;
  ...
  estatistico(bytes: Buffer): number {
  ...
  phi(bits: number[], m: number, n: number): number {
    const total = 1 << m;
    const contagem: number[] = new Array(total).fill(0);
    const janelas = n - m + 1;
