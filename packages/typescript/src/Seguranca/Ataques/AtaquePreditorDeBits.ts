...
interface OpcoesPreditorDeBits {
  tamanho?: number;
  contexto?: number;
  limiteTaxa?: number;
}
 ...
 */
export class AtaquePreditorDeBits implements AtaqueInterface {
  private readonly tamanho: number;
  private readonly contexto: number;
  private readonly limiteTaxa: number;

  constructor({ tamanho = 16384, contexto = 8, limiteTaxa = 0.55 }: OpcoesPreditorDeBits = {}) {
  ...
  executar(alvo: AlvoCriptografico): ResultadoAtaque {
    ...
    const uns = new Array<number>(total).fill(0);
    const cont = new Array<number>(total).fill(0);
  ...
  contextoDe(bits: number[], i: number, k: number): number {
    let ctx = 0;
    for (let j = i - k; j < i; j++) {
      ctx = (ctx << 1) | bits[j];
    }
