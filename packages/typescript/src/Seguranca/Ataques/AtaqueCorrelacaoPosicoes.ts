...
interface OpcoesCorrelacaoPosicoes {
  amostras?: number;
  tamanhoBloco?: number;
  limiteCorrelacao?: number;
}
 ...
 */
export class AtaqueCorrelacaoPosicoes implements AtaqueInterface {
  private readonly amostras: number;
  private readonly tamanhoBloco: number;
  private readonly limiteCorrelacao: number;

  constructor({
    amostras = 3000,
    tamanhoBloco = 32,
    limiteCorrelacao = 0.15,
  }: OpcoesCorrelacaoPosicoes = {}) {
  ...
  pearson(blocos: Buffer[], a: number, b: number): number {
