 ...
 */
export class AtaqueBateriaEstatistica implements AtaqueInterface {
  private readonly tamanho: number;
  private readonly zLimite: number;

  constructor({ tamanho = 16384, zLimite = 4.0 }: { tamanho?: number; zLimite?: number } = {}) {
  ...
  executar(alvo: AlvoCriptografico): ResultadoAtaque {
    ...
    const unsPorPos = new Array<number>(8).fill(0);
    const contaPorPos = new Array<number>(8).fill(0);
    ...
    const posViesadas: Record<number, number> = {};
    for (let b = 0; b < 8; b++) {
    ...
    return new ResultadoAtaque(
      ...
      false,
      'info',
      `monobit z=${zMonobit.toFixed(2)}, runs z=${((dados.runs_z as number | undefined) ?? 0.0).toFixed(2)}, blocos chi2=${((dados.blocos_chi2 as number | undefined) ?? 0.0).toFixed(1)} - todos abaixo do limite z=${this.zLimite.toFixed(0)}`,
      dados,
    );
  }

  private paraBits(bytes: Buffer): number[] {
    const bits: number[] = [];
