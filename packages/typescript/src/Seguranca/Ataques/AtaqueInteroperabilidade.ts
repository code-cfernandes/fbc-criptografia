...
const CHAVE_FIXA = 'K'.repeat(32);

interface VetorKeystream {
  iv: string;
  proposito: 'enc' | 'mac';
  tamanho: number;
  esperado: string;
...
interface VetorToken {
  texto: string;
  token: string;
}

const TOKENS: VetorToken[] = [
...
export class AtaqueInteroperabilidade implements AtaqueInterface {
  nome(): string {
  ...
  executar(alvo: AlvoCriptografico): ResultadoAtaque {
    ...
    for (const v of VETORES) {
      const ks = alvo.gerarKeystreamBruto(
        Buffer.from(CHAVE_FIXA, 'utf8'),
        Buffer.from(v.iv, 'hex'),
        v.proposito,
        v.tamanho,
      );
      if (ks.toString('hex') !== v.esperado) {
        falhas.push(`keystream iv=${v.iv} prop=${v.proposito} tam=${v.tamanho}`);
      }
    }
    ...
    for (const t of TOKENS) {
      try {
        if (alvo.decrypt(t.token) !== t.texto) {
          falhas.push(`token não decifrou para ${JSON.stringify(t.texto)}`);
        }
      } catch {
        falhas.push(`token rejeitado: ${JSON.stringify(t.texto)}`);
      }
    }
    ...
    if (falhas.length > 0) {
      return new ResultadoAtaque(
        ...
        true,
        'alta',
        `${falhas.length} divergência(s) de interoperabilidade: ${falhas.slice(0, 5).join('; ')}`,
        { falhas },
      );
    }
    ...
    return new ResultadoAtaque(
      ...
      false,
      'info',
      `${VETORES.length} vetores de keystream e ${TOKENS.length} tokens de referência conferem`,
    );
  }
}
