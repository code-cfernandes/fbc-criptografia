 ...
 */
export class AtaqueFoldEstrutural implements AtaqueInterface {
  private readonly amostras: number;
  private readonly tamanhoBloco: number;

  constructor({ amostras = 5000, tamanhoBloco = 32 }: { amostras?: number; tamanhoBloco?: number } = {}) {
  ...
  executar(alvo: AlvoCriptografico): ResultadoAtaque {
    ...
    const margem = (divisor: number): number =>
