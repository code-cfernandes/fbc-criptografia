 ...
 */
export class AtaqueCiphertextEstatistico implements AtaqueInterface {
  private readonly amostras: number;
  private readonly tamanhoTexto: number;
  ...
  constructor({
    ...
    tamanhoTexto = 64,
    zLimite = 4,
  }: { amostras?: number; tamanhoTexto?: number; zLimite?: number } = {}) {
  ...
  executar(alvo: AlvoCriptografico): ResultadoAtaque {
    const contagem = new Array<number>(256).fill(0);
    const bytes: number[] = [];

    for (let s = 0; s < this.amostras; s++) {
      const texto = randomBytes(this.tamanhoTexto);
      const token = alvo.encrypt(texto);
      const campos = alvo.decompor(alvo.base64urlDecode(token.slice(alvo.prefixo().length)));
