...
import { randomInt, arrayRandKey } from '../Util.ts';
 ...
 */
export class AtaqueAdulteracao implements AtaqueInterface {
  private readonly tentativas: number;

  constructor({ tentativas = 500 }: { tentativas?: number } = {}) {
  ...
  executar(alvo: AlvoCriptografico): ResultadoAtaque {
    ...
    const aceitosIndevidamente: string[] = [];
