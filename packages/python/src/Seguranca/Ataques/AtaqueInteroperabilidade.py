...
class AtaqueInteroperabilidade:
    ...
    def executar(self, alvo) -> ResultadoAtaque:
        ...
        for v in VETORES:
            ...
            if ks.hex() != v["esperado"]:
                falhas.append(f"keystream iv={v['iv']} prop={v['proposito']} tam={v['tamanho']}")

        for t in TOKENS:
            try:
                if alvo.decrypt(t["token"]) != t["texto"]:
                    falhas.append(f"token não decifrou para {t['texto']!r}")
            except Exception:
                falhas.append(f"token rejeitado: {t['texto']!r}")
        ...
        if falhas:
            return ResultadoAtaque(
                ...
                True,
                "alta",
                f"{len(falhas)} divergência(s) de interoperabilidade: {'; '.join(falhas[:5])}",
                {"falhas": falhas},
            )
        ...
        return ResultadoAtaque(
            ...
            False,
            "info",
            f"{len(VETORES)} vetores de keystream e {len(TOKENS)} tokens de referência conferem",
        )
